...
export interface AgentActivitySession {
  ...
  /**
   * Read projection of the independent daemon-owned AgoraxModeActivation.
   * The session does not own this lifecycle; activity-core normalizes it into
   * its dedicated activation slice.
   */
  agoraxModeActivation: AgentActivityAgoraxModeActivation | null;
  imported: boolean;
  visible: boolean;
  resumable: boolean;
...
export interface AgentActivityCreateSessionInput {
  ...
  initialAgoraxModeActivation?: AgentActivityInitialAgoraxModeActivation | null;

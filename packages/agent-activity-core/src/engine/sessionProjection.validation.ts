...
function optionalSessionFieldsMatch(
  ...
  return (
    ...
    optionalAgoraxModeActivation(
      value.agoraxModeActivation,
      agentSessionId,
      workspaceId
    ) &&
...
function optionalAgoraxModeActivation(
  value: unknown,
  agentSessionId: string,
  workspaceId: string
): boolean {
  ...
  return (
    ...
    trimmedString(value.agentSessionId) === agentSessionId &&
    isOneOf(value.status, AGORAX_MODE_STATUSES) &&
    isFiniteNumber(value.createdAtUnixMs) &&
    isFiniteNumber(value.updatedAtUnixMs) &&
    Boolean(trimmedString(revision.activationId)) &&
    isFiniteNumber(revision.revision) &&
    isOneOf(revision.status, AGORAX_MODE_STATUSES) &&
    isOneOf(revision.source, AGORAX_MODE_SOURCES) &&
...
const AGORAX_MODE_STATUSES = ["active", "inactive"] as const;
const AGORAX_MODE_SOURCES = ["slash_command", "badge_remove"] as const;

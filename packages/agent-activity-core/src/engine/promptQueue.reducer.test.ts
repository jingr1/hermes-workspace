...
test("queued Agorax capability reference remains structured through delivery", () => {
  ...
  const capabilityRefs = [
    { capability: "agorax" as const, source: "slash_command" as const }
  ];
  const queued = reduce(
    loaded.state,
    {
      ...submit("prompt-agorax"),
      capabilityRefs
    },
    lifecycle
  );

...
test("session snapshots preserve monotonic message and AgoraxMode revisions", () => {
  const activation = {
    agentSessionId: "session-1",
    createdAtUnixMs: 1,
    currentRevision: {
  ...
  const current = {
    ...session(null, 2),
    messageVersion: 5,
    agoraxModeActivation: activation
  };
  let state = reduce(createInitialSessionLifecycleState(), {
    type: "session/snapshotReceived",
    sessions: [current]
  ...
  assert.equal(projected?.agoraxModeActivation?.currentRevision.revision, 2);
});

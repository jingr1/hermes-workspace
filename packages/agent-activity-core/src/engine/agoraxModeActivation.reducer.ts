...
import type {
  AgoraxModeActivationState,
  AgoraxModeActivationUpdateRecord
} from "./agoraxModeActivation.types.ts";

const NO_COMMANDS: readonly EngineCommand[] = [];
const UPDATE_TIMEOUT_MS = 15_000;

interface AgoraxModeActivationReducerContext {
  sessionsById: Readonly<Record<string, CanonicalAgentSession>>;
}

export function createInitialAgoraxModeActivationState(): AgoraxModeActivationState {
  return {
...
export function agoraxModeActivationReducer(
  state: AgoraxModeActivationState,
  intent: EngineIntent,
  context: AgoraxModeActivationReducerContext
): EngineReducerResult<AgoraxModeActivationState> {
  switch (intent.type) {
    case "agoraxMode/draftSet":
      return setDraft(state, intent);
    case "agoraxMode/updateRequested":
    ...
    case "engine/commandResult":
      if (intent.commandType === "agoraxMode/update") {
...
function setDraft(
  state: AgoraxModeActivationState,
  intent: Extract<EngineIntent, { type: "agoraxMode/draftSet" }>
): EngineReducerResult<AgoraxModeActivationState> {
...
function trackPendingCreate(
  ...
  if (
    intent.mode !== "new" ||
    !intent.initialAgoraxModeActivation ||
    !intent.agoraxModeDraftKey?.trim()
  ) {
    return unchanged(state);
  }
  const agentSessionId = intent.agentSessionId.trim();
  const draftKey = intent.agoraxModeDraftKey.trim();
  ...
  const effect =
    normalizePreferenceWithLegacy(
      intent.initialAgoraxModeActivation.effect,
      intent.initialAgoraxModeActivation.orchestrationIntensity
    ) ??
    draft.effect ??
    draft.orchestrationIntensity;
  const speed =
    normalizePreference(intent.initialAgoraxModeActivation.speed) ?? draft.speed;
  return {
    commands: NO_COMMANDS,
    state: {
      ...state,
...
function settlePendingCreate(
  ...
  if (intent.outcome === "timedOut" || intent.outcome === "succeeded") {
    ...
    const reconcileCommandId = `agorax-mode-create-reconcile:${pending.requestId}`;
    return {
      commands: [
        {
...
function requestUpdate(
  state: AgoraxModeActivationState,
  intent: Extract<EngineIntent, { type: "agoraxMode/updateRequested" }>,
  sessionsById: Readonly<Record<string, CanonicalAgentSession>>
): EngineReducerResult<AgoraxModeActivationState> {
  const agentSessionId = intent.agentSessionId.trim();
  ...
  const record: AgoraxModeActivationUpdateRecord = {
    agentSessionId,
    commandId,
    errorCode: null,
    errorMessage: null,
      ...
      ? [
          {
            ...
            commandId: `agorax-mode-reconcile:${entry.commandId}`,
            live: false,
            scope: "state",
            timeoutMs: UPDATE_TIMEOUT_MS,
            type: "session/reconcile",
      ...
      updatesBySessionId: {
        ...
        [entry.agentSessionId]: {
          ...
          reconcileCommandId: uncertain
            ? `agorax-mode-reconcile:${entry.commandId}`
            : null,
...
function hydrateSessions(
  ...
  for (const rawId of sessionIds) {
    ...
    const session = sessionsById[agentSessionId];
    if (!session) continue;
    const activation = session.agoraxModeActivation
      ? cloneActivation(session.agoraxModeActivation)
      : null;
...
function settleOwnedReconcile(
  ...
  if (update) {
    const activation =
      sessionsById[update.agentSessionId]?.agoraxModeActivation ?? null;
    if (
      intent.outcome === "succeeded" &&
      ...
      state: {
        ...
        updatesBySessionId: {
          ...
          [update.agentSessionId]: {
            ...
            errorCode:
              intent.outcome === "succeeded"
                ? "agorax_mode_update_not_applied"
                : intent.errorCode?.trim() || "agorax_mode_reconcile_failed",
            errorMessage: intent.errorMessage?.trim() || null,
            reconcileCommandId: null,
            updateStatus: "failed"
          }
  ...
  const activation =
    sessionsById[pending.agentSessionId]?.agoraxModeActivation ?? null;
...
function updateSemanticallyApplied(
  update: AgoraxModeActivationUpdateRecord,
  activation: AgentActivitySession["agoraxModeActivation"]
): boolean {
  return Boolean(
    activation &&
...
function validUpdateResult(
  value: unknown,
  entry: AgoraxModeActivationUpdateRecord
): {
  activation: NonNullable<AgentActivitySession["agoraxModeActivation"]>;
} | null {
  ...
  const candidate = activation as NonNullable<
    AgentActivitySession["agoraxModeActivation"]
  >;
...
function activationFromCreateResult(
  value: unknown,
  pending: AgoraxModeActivationState["pendingCreatesBySessionId"][string]
): NonNullable<AgentActivitySession["agoraxModeActivation"]> | null {
  if (!value || typeof value !== "object") return null;
  const session = (value as { session?: unknown }).session;
  if (!session || typeof session !== "object") return null;
  const candidate = session as Partial<AgentActivitySession>;
  const activation = candidate.agoraxModeActivation;
...
function sameActivation(
  left: AgentActivitySession["agoraxModeActivation"] | undefined,
  right: AgentActivitySession["agoraxModeActivation"]
): boolean {
  return (
    left === right ||
    (left?.id === right?.id &&
...
function updateEffect(update: AgoraxModeActivationUpdateRecord): number | null {
  return (

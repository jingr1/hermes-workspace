...
import { agentActivityComposerOptionsFromDaemonResult } from "./composerOptions.ts";

test("maps daemon composer options into the canonical activity contract", () => {
  const options = agentActivityComposerOptionsFromDaemonResult("codex", {
    ...
    capabilityCatalog: [],
    provider: "codex"
  } satisfies DaemonProviderComposerOptionsResponse);
...
test("keeps fallback slash commands when effects are absent", () => {
  ...
  const options = agentActivityComposerOptionsFromDaemonResult(
    "acp:hermes",
    response
  );
...
test("accepts agorax-injected and connector skill source kinds", () => {
  const response = {
    behavior: {},
    capabilityCatalog: [],
    commands: [],
  ...
  assert.deepEqual(
    options.skills.map((skill) => skill.sourceKind),
    ["agorax-injected", "connector"]
  );
});

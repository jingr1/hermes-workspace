...
if ([string]::IsNullOrWhiteSpace($WorkspacePath)) {
  $WorkspacePath = "/home/$wslUser/agorax"
}
...
if ($sessionExists -and -not $Restart) {
  ...
  Write-Host "Workspace URL: http://localhost:6734"
  return
}
...
Write-Host "Started Hermes Gateway + Agorax in tmux session '$SessionName'."
Write-Host "Workspace URL: http://localhost:6734"

...
export const Route = createFileRoute(
  '/api/agents/$agentId/engine/session/$sessionId/pin',
)({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        if (typeof body?.pinned !== 'boolean') {
          return json({ error: 'pinned boolean is required' }, { status: 400 })
        }
        ...
        try {
          const result = await client.updatePin(identity.agentSessionId, body.pinned)
          return json({
            result,

...
describe('mapManagedAgentActivitySnapshot', () => {
  it('maps daemon canonical activity JSON into Engine DTOs', () => {
    const snapshot = mapManagedAgentActivitySnapshot({
      ...
      turns: [{ TurnID: 'turn-1', AgentSessionID: 'session-1', Phase: 'waiting', Origin: 'user_prompt', StartedAtUnixMS: 1, UpdatedAtUnixMS: 2, ErrorCode: 'provider_error', ErrorMessage: 'provider rejected request', ErrorDetail: 'HTTP 429: quota exhausted' }],
    ...
    expect(snapshot?.turns[0]?.error).toEqual({ code: 'provider_error', message: 'provider rejected request', detail: 'HTTP 429: quota exhausted' })
  })

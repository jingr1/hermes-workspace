...
describe('chat-event-bus', () => {
  it('filters by sessionKey when given a string filter', () => {
    ...
    const unsubscribe = subscribeToChatEvents((evt) => received.push(evt), 'sess-1')
  ...
  it('filters by roomId when given an object filter', () => {
    const received: Array<{ event: string; data: Record<string, unknown> }> = []
    const unsubscribe = subscribeToChatEvents((evt) => received.push(evt), { roomId: 'room-42' })
  ...
  it('filters by scope when given an object filter', () => {
    const received: Array<{ event: string; data: Record<string, unknown> }> = []
    const unsubscribe = subscribeToChatEvents((evt) => received.push(evt), { scope: 'global' })
  ...
  it('combines sessionKey and roomId filters', () => {
    ...
    publishChatEvent('msg', { sessionKey: 'sess-1', roomId: 'room-42', text: 'both' })
    publishChatEvent('msg', { sessionKey: 'sess-1', roomId: 'room-99', text: 'wrong-room' })
    publishChatEvent('msg', { sessionKey: 'sess-2', roomId: 'room-42', text: 'wrong-session' })

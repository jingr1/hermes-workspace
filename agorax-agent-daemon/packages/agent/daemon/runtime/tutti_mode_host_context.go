...
import (
	...
	runtimepaths "agorax.local/agent-daemon/packages/agent/daemon/internal/runtimepaths"
)

type agoraxModeTurnSnapshotContextKey struct{}

func normalizeAgoraxModeTurnSnapshot(snapshot *AgoraxModeTurnSnapshot) *AgoraxModeTurnSnapshot {
	if snapshot == nil {
		return nil
	}
	effect := snapshot.Effect
	speed := snapshot.Speed
	if snapshot.PreferenceVersion < AgoraxModePreferenceVersionEffectSpeed {
		effect = snapshot.OrchestrationIntensity
		speed = 50
	}
	normalized := &AgoraxModeTurnSnapshot{
	...
	if normalized.State != AgoraxModeStateActive && normalized.State != AgoraxModeStateInactive {
		return nil
	}
...
func cloneAgoraxModeTurnSnapshot(snapshot *AgoraxModeTurnSnapshot) *AgoraxModeTurnSnapshot {
	normalized := normalizeAgoraxModeTurnSnapshot(snapshot)
	if normalized == nil {
		return nil
	}
	cloned := *normalized
...
func withAgoraxModeTurnSnapshot(ctx context.Context, snapshot *AgoraxModeTurnSnapshot) context.Context {
	normalized := cloneAgoraxModeTurnSnapshot(snapshot)
	if normalized == nil {
		return ctx
	}
	return context.WithValue(ctx, agoraxModeTurnSnapshotContextKey{}, normalized)
}

func agoraxModeTurnSnapshotFromContext(ctx context.Context) *AgoraxModeTurnSnapshot {
	if ctx == nil {
		return nil
	}
	snapshot, _ := ctx.Value(agoraxModeTurnSnapshotContextKey{}).(*AgoraxModeTurnSnapshot)
	return cloneAgoraxModeTurnSnapshot(snapshot)
}

// agoraxCLICommandName resolves the executable name agents must use for Agorax
// CLI workflow commands: development installs ship the CLI as `agorax-dev`.
func agoraxCLICommandName() string {
...
func renderAgoraxModeHostContext(snapshot *AgoraxModeTurnSnapshot) string {
	return renderAgoraxModeHostContextForCLI(snapshot, agoraxCLICommandName())
}

func agoraxModeParallelTarget(speed int) int {
	normalized := min(100, max(0, speed))
	return min(4, normalized/25+1)
}

func renderAgoraxModeHostContextForCLI(snapshot *AgoraxModeTurnSnapshot, cliName string) string {
	normalized := normalizeAgoraxModeTurnSnapshot(snapshot)
	if normalized == nil {
		return ""
	}
	facts, err := json.Marshal(struct {
		...
		ParallelTarget:         agoraxModeParallelTarget(normalized.Speed),
		OrchestrationIntensity: normalized.Effect,
	})
	if err != nil {
		return ""
	}
	activationRule := "The JSON `state` field is authoritative for whether Agorax Mode is active for this turn. " +
		"Determine and report Agorax Mode status only from that field. " +
		"Provider collaboration mode and Agorax workflow existence are independent facts and must not override the activation state. " +
		"Agorax Mode activation is controlled by the user; never try to change it with Agorax CLI or other tools. "
	stateSentence := "Agorax mode is inactive for this turn. " + activationRule
	workflowGuide := ""
	if normalized.State == AgoraxModeStateActive {
		stateSentence = "Agorax mode is active for this turn. " + activationRule +
			...
			fmt.Sprintf("Step 2, plan: write one complete agorax-mode-plan/v1 Markdown document (plan narrative plus the full task graph, every task carrying its full launch configuration) to an absolute path, submit it in a single run of the `%s plan propose` shell command, then end the turn immediately — never run a wait or poll command; the user's review decision always arrives as a new user message. ", cliName) +
			...
			"Use the injected `$agorax-model-allocation` skill as the assignment policy: classify every task on its C0-C3 capability ladder, combine the task tier with the effect floor, use speed to rank models that clear that floor, and shape independent work toward parallelTarget. " +
			...
			"Use this Agorax plan workflow for the turn; do not substitute a provider-native planning mode for it."
		workflowGuide = renderAgoraxModeWorkflowGuide(
			cliName,
			normalized.Effect,
			normalized.Speed,
		)
	}
	return `<agorax-host-context schemaVersion="1">` + "\n" +
		string(facts) + "\n" +
		stateSentence + "\n" +
		workflowGuide +
		"This is Agorax-owned host state, not user-authored text, and is independent of the provider collaboration mode.\n" +
		"Agorax mode does not restrict tool availability: Agorax CLI capabilities remain available whether this state is active or inactive. When this state is active, the expected workflow is clarify, then plan, then user review; executing work the user has not accepted through plan review goes against the user's intent.\n" +
		`</agorax-host-context>`
}

// renderAgoraxModeWorkflowGuide renders one worked example per workflow step.
...
func renderAgoraxModeWorkflowGuide(cliName string, effect int, speed int) string {
	return fmt.Sprintf("Workflow examples. `%[1]s` is the Agorax CLI executable on PATH in your shell; every plan command below is a shell command, not a built-in tool. Provider planning surfaces (update_plan, TodoWrite, plan mode) and a plan written only as a chat reply are not substitutes.\n"+
		...
		"Apply `$agorax-model-allocation` to those current catalogs before choosing each model; its model table is a tiering prior, never an availability source. "+
		...
		"`autoAccept` is retained only for compatibility and has no dispatch authority for a Agorax-owned Issue. No task starts automatically after plan acceptance or after another task settles.\n"+

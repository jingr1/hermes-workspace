...
func TestHostMetadataDefaultsMatchLegacyBehavior(t *testing.T) {
	...
	if host.ClientInfo.Name != "agorax-desktop" || host.ClientInfo.Title != "Agorax" || host.ClientInfo.Version != "0.1.0" {
		t.Fatalf("client info = %#v, want legacy tsh desktop metadata", host.ClientInfo)
	}
	if got := workspaceEnv(Session{RoomID: "workspace-1"}, host); len(got) != 1 || got[0] != "AGORAX_WORKSPACE_ID=workspace-1" {
		t.Fatalf("workspace env = %#v, want AGORAX_WORKSPACE_ID", got)
	}
	if got := openclawGatewayChatSessionKey(Session{AgentSessionID: "session-1"}, host); got != "agent:main:agorax-session-1" {
		t.Fatalf("session key = %q, want legacy tsh key", got)
	}
	if got := openclawGatewayChatSessionKey(Session{}, host); got != "agent:main:agorax-desktop" {
...
func TestHostMetadataHelpersDoNotApplyLegacyDefaults(t *testing.T) {
	...
	if clientInfo["name"] == "agorax-desktop" || clientInfo["title"] == "Agorax" || clientInfo["version"] == "0.1.0" {

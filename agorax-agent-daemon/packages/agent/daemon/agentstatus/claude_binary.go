...
// package, ~230MB per platform, essentially incompressible with gzip). agorax-agentd
...
// AGORAX_CLAUDE_CODE_FALLBACK_EXECUTABLE for the sidecar (see
...
import (
	...
	"agorax.local/agent-daemon/packages/agent/daemon/usercommand"
	agoraxstate "agorax.local/agent-daemon/packages/agent/daemon/agoraxstate"
)

const (
	// claudeCodeBinaryBaseURLEnv, when set, enables CDN download of
	// zstd-compressed claude binaries (layout: <base>/<claudeVersion>/claude-<platform>.zst).
	// Agorax has no default CDN — unset means npm-only provisioning.
	claudeCodeBinaryBaseURLEnv = "AGORAX_CLAUDE_CODE_BINARY_BASE_URL"

	// claudeCodeStateRelDir is the Agorax-state-relative root for provisioned
...
// not only the intended publication directory. This prevents a Agorax launcher
...
func (s Service) downloadClaudeCodeBinary(
	...
	if base != "" {
		...
		cdnErr := s.installClaudeCodeBinaryFromCDN(cdnCtx, descriptor, finalPath, base)
		cancelCDN()
		if cdnErr == nil {
			return "cdn", nil
		}
		...
		slog.Warn(
			...
			"event", "agorax.claude_code_binary.cdn_failed",
	...
	return "", fmt.Errorf("claude code binary unavailable via npm (set %s for an Agorax-owned CDN): %w", claudeCodeBinaryBaseURLEnv, npmErr)
}
...
func (s Service) claudeCodeStateRoot() string {
	...
	if stateDir == "" {
		stateDir = agoraxstate.DefaultStateDir()
	}

...
const (
	...
	// always Agorax-owned and writable by the daemon's user. See withAgentNPMCache.
...
// access. An explicit AGORAX_AGENT_NPM_REGISTRY pins a single registry with no
// fallback.
...
// a dedicated Agorax-owned cache sidesteps the broken global cache entirely.

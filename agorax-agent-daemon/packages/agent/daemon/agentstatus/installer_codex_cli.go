...
func (s Service) runManagedNPMPackageAction(
	...
	// node, whose prefix (~/.agorax/app-runtimes/.../node) is NOT on the binary
	...
	// restored in place. Falls back to the selected Agorax install directory above
	...
	// Pin a dedicated, Agorax-owned npm cache instead of the user's global ~/.npm,
...
func (s Service) resolveManagedNPMInstallerNodeRuntime(
	...
	if err != nil {
		...
		return "", "", nil, fmt.Errorf("Node.js/npm not found on PATH and Agorax managed Node is disabled (set AGORAX_APP_RUNTIME_CATALOG for an Agorax-owned catalog): %w", err)
	}
	...
	if npmPath == "" {
		...
		return "", "", nil, fmt.Errorf("Agorax managed Node runtime did not provide npm and npm was not found on PATH")
	}

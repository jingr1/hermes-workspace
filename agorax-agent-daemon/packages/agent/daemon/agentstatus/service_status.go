...
func (s Service) statusForSpec(
	...
	defer func() {
		slog.Info(
			...
			"event", "agorax.agent_provider.status_detection.completed",
	...
	if status.ActiveAction != nil {
		...
		slog.Info(
			...
			"event", "agorax.agent_provider.status.active_action_attached",
			"provider", spec.Provider,
			"availability", status.Availability.Status,
			"reasonCode", status.Availability.ReasonCode,
	...
	if isClaudeStatusSpec(spec) {
		slog.Info(
			...
			"event", "agorax.agent_provider.status.checked",
			"provider", spec.Provider,
			"availability", status.Availability.Status,
			"reasonCode", status.Availability.ReasonCode,

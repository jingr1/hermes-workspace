...
import (
	...
	agentproviderbiz "agorax.local/agent-daemon/packages/agent/daemon/agentprovider"
	externalagentregistry "agorax.local/agent-daemon/packages/agent/daemon/externalagentregistry"
...
type Service struct {
	...
	// ClaudeCodeStateDir overrides the Agorax state root that hosts the managed
	...
	ClaudeAccountUsageProbe func(context.Context, ClaudeSDKAccountUsageProbeInput) ClaudeSDKAccountUsageProbeResult
...
func (s Service) List(ctx context.Context, input ListInput) (snapshot Snapshot, err error) {
	...
	defer func() {
		slog.Info(
			...
			"event", "agorax.agent_provider.status_list.completed",
...
func (s Service) runInstallAction(ctx context.Context, spec ProviderSpec, result RunActionResult) (RunActionResult, error) {
	...
	if shared {
		slog.Info(
			...
			"event", "agorax.agent_provider.install.coalesced",
			"provider", spec.Provider,
		)
	}
...
func (s Service) runInstallActionOnce(ctx context.Context, spec ProviderSpec, result RunActionResult) (RunActionResult, error) {
	...
	if isCodexStatusSpec(spec) && strings.TrimSpace(runtimeResolution.CLIPath) != "" {
		...
		if probe.Status == ProbeReady {
			// A present, protocol-capable Codex below Agorax's support floor needs

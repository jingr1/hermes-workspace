...
// MinSupportedCodexVersion is the lowest Codex CLI version Agorax supports.
//
// Service-compatibility-derived: older Codex releases can still expose the
// app-server methods Agorax calls while being rejected by the upstream service
// before current model requests complete. The floor therefore tracks the
// current stable Codex release verified by Agorax's app-server contract tests.
//

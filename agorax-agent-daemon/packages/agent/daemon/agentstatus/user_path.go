...
// UserPathAdapter publishes a Agorax-owned executable directory to the current
...
// older Agorax releases may remain in the legacy .local npm prefix. Keeping the
...
func (s Service) publishManagedInstallBinaryDir(ctx context.Context, binaryPath string) error {
	...
	if managedDir == "" {
		// Version-manager and user-managed installs remain outside Agorax's PATH
		// ownership boundary.
		return nil
	}
...
// publishDetectedManagedBinaryDirs makes an already-installed Agorax-managed npm

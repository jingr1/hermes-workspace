...
import (
	...
	managedruntime "agorax.local/agent-daemon/packages/agent/daemon/managedruntime"
)
...
func (s Service) runManagedClaudeCodeInstaller(ctx context.Context) (InstallCommandResult, error) {
	...
	slog.Info(
		...
		"event", "agorax.claude_code.managed_install.started",
	)
	status, err := s.EnsureClaudeCodeBinary(ctx)
	if err != nil {
		slog.Warn(
			"claude code managed runtime install failed",
			"event", "agorax.claude_code.managed_install.failed",
	...
	slog.Info(
		...
		"event", "agorax.claude_code.managed_install.completed",
		"durationMs", time.Since(startedAt).Milliseconds(),
		"source", status.Source,
...
func (s Service) runReleaseBinaryInstaller(
	...
	archiveFile, err := os.CreateTemp("", "agorax-provider-archive-*"+archiveSuffix(asset.URL))
	if err != nil {
		return InstallCommandResult{ExitCode: 1}, err
	}
...
func (s Service) runExternalAgentRegistryNPMInstaller(ctx context.Context, provider string, spec InstallerSpec) (InstallCommandResult, error) {
	...
	if err != nil {
		// Prefer system Node/npm when Agorax has no managed runtime catalog.
		commandResolver := s.commandResolver()
		if npmPath := strings.TrimSpace(resolveBinaryWithResolver(commandResolver, []string{npmBinaryName()}, nil)); npmPath != "" {
			appRuntime = managedruntime.ResolvedRuntime{
				NPM:          npmPath,
				Node:         firstNonBlank(resolveBinaryWithResolver(commandResolver, []string{nodeBinaryName()}, nil), nodeBinaryName()),
				EnvOverrides: nil,
			}
			err = nil
		} else {
			return InstallCommandResult{
				ExitCode: 1,
				Stderr: fmt.Sprintf(
					"Node.js/npm not found on PATH and Agorax managed Node is disabled (set AGORAX_APP_RUNTIME_CATALOG for an Agorax-owned catalog): %v",
					err,
				),
			}, nil
		}
	...
	// Use a dedicated, Agorax-owned npm cache inside the install prefix rather than
...
func ensureWritableInstallDir(dir string) error {
	...
	file, err := os.CreateTemp(dir, ".agorax-install-test-*")
	if err != nil {

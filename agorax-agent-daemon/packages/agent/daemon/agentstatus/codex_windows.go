...
// launchable by agorax-agentd. Windows permits the packaged Codex app to execute its
// own resources, but a child process spawned by a non-AppX Agorax process gets
// ERROR_ACCESS_DENIED for the same path. Copying the standalone CLI binary to
// Agorax's writable runtime cache preserves the user's installed version while
// giving CreateProcess a normal filesystem path.
func materializeCodexWindowsAppsLauncher(path string) string {
	cacheRoot, err := agoraxstate.DefaultAgentRuntimeDir()
	if err != nil {
		return path
	}
...
func materializeCodexWindowsAppsLauncherAt(path string, cacheRoot string) string {
	...
	if err := os.MkdirAll(destinationDir, 0o755); err != nil {
		slog.Warn("codex AppX launcher cache directory unavailable",
			"event", "agorax.codex.appx_launcher.materialize_failed",
			"source", path,
	...
	slog.Info("codex AppX launcher materialized",
		"event", "agorax.codex.appx_launcher.materialized",
		"source", path,
		"destination", destination,

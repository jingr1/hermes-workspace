...
const claudeSDKSidecarCommandEnv = "AGORAX_CLAUDE_SDK_SIDECAR_COMMAND"
const claudeSDKSidecarEntryPathEnv = "AGORAX_CLAUDE_SDK_SIDECAR_ENTRY_PATH"
...
func (s Service) resolveClaudeCodeProviderSpec(ctx context.Context, spec ProviderSpec, requireManagedRuntime bool) ProviderSpec {
	...
	if entry == "" {
		...
		spec.AdapterBinaryNames = []string{"agorax-claude-sdk-sidecar-missing"}
...
func resolvedExistingManagedNodeRuntime(root string, environ func() []string) (managedruntime.ResolvedRuntime, bool) {
	...
	return managedruntime.ResolvedRuntime{
		...
		EnvOverrides: []string{
			"AGORAX_APP_RUNTIME_ROOT=" + root,
			"AGORAX_APP_NODE=" + nodePath,
			"AGORAX_APP_NPM=" + npmPath,

...
const windowsLauncherMarker = "rem Agorax managed agent command v1"
...
func (e Entry) classifyUserEntry() (userEntryKind, error) {
	...
	if err != nil {
		// The file exists but is not a Agorax launcher: a user-owned command
		// with the same name.
...
func readWindowsLauncher(path string) (string, bool, error) {
	...
	if len(lines) < 4 || lines[0] != "@echo off" || lines[1] != windowsLauncherMarker {
		return "", true, errors.New("file is not a Agorax managed command launcher")
	}

...
// Entry describes the two stable command hops owned by Agorax. Runtime updates
...
const (
	...
	// userEntryManaged: the entry is Agorax's and points at the stable path.
	userEntryManaged
...
func (e Entry) Verify() error {
	...
	if kind != userEntryManaged {
		// No Agorax user command is installed (absent or foreign). The managed
...
// user-command entry is owned by Agorax. A foreign occupant is preserved
...
func newEntryQuarantinePath(path string) (string, error) {
	temporary, err := os.CreateTemp(filepath.Dir(path), "."+filepath.Base(path)+".agorax-unpublish-*")
	if err != nil {
		return "", err
	}

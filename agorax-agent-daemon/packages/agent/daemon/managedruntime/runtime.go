...
import (
	...
	"agorax.local/agent-daemon/packages/agent/daemon/httpx"
...
const agoraxAppRuntimeRootEnv = "AGORAX_APP_RUNTIME_ROOT"
const agoraxAppRuntimeCacheRootEnv = "AGORAX_APP_RUNTIME_CACHE_ROOT"
...
func (r DefaultResolver) runtimeRoot() string {
	...
	if root == "" {
		root = strings.TrimSpace(envValue(r.environ(), agoraxAppRuntimeRootEnv))
	}
	if root == "" {
...
func (r DefaultResolver) resolvedRuntimeForComponents(root string, components []string) (ResolvedRuntime, error) {
	...
	for _, component := range components {
		...
		case "python":
			...
			envOverrides = append(envOverrides, "AGORAX_APP_PYTHON="+python)
		case "node":
			...
			envOverrides = append(envOverrides, "AGORAX_APP_NODE="+node, "AGORAX_APP_NPM="+npm)
		case "rtk":
			...
			envOverrides = append(envOverrides, "AGORAX_APP_RTK="+rtk)
		}
	}
	...
	resolved.EnvOverrides = append(
		[]string{agoraxAppRuntimeRootEnv + "=" + root},
		envOverrides...,
	)
	resolved.EnvOverrides = append(
...
func (r DefaultResolver) ensureRuntimeProfile(ctx context.Context, root string, profile string) error {
	...
	if catalogSource == "" {
		return fmt.Errorf(
			"managed app runtime is unavailable at %s; install Node.js on PATH, or set %s to an Agorax-owned catalog (no default remote catalog)",
			root,
			agoraxAppRuntimeCatalogEnv,
		)
	}
...
func (r DefaultResolver) runtimeProfileComponentNames(ctx context.Context, profile string) ([]string, error) {
	...
	if catalogSource == "" {
		return nil, fmt.Errorf(
			"managed app runtime catalog is not configured; set %s to an Agorax-owned catalog, or use system Node/npm on PATH",
			agoraxAppRuntimeCatalogEnv,
		)
	}
	platformArch := appRuntimePlatformArch(runtime.GOOS, runtime.GOARCH)
...
func (r DefaultResolver) runtimeCatalogSource() string {
	// Opt-in only: never default to a third-party CDN.
	return strings.TrimSpace(envValue(r.environ(), agoraxAppRuntimeCatalogEnv))
}
...
func DefaultRoot(env []string) string {
	cacheRoot := strings.TrimSpace(envValue(env, agoraxAppRuntimeCacheRootEnv))
	if cacheRoot == "" {
		cacheRoot = filepath.Join(agoraxstate.DefaultStateDir(), "app-runtimes")
	}
